...
void BattleUnitRenderer::LevelChangeStart(GameEngineLevel* _PrevLevel)
{
	...
	if (PlayerRenderer_ == nullptr)
	{
		...
		//»ó´ë Æ®·¹ÀÌ³Ê
		//±è¿¹³ª : ÀÓ½Ã·Î ¶óÀÌ¹ú ÀÌ¹ÌÁö¸¦ ³Ö¾ú¾î¿ä »óÈ²¿¡ µû¶ó ¾ß»ý Æ÷ÄÏ¸óÃ³·³ ´Ù¸¥ ·£´õ·¯ ¶ß°Ô ÇÏ¸é µÉ°Å°°¾Æ¿ä
	...
	if (BattleDataR_->IsWild() == true) // ¿©±â¼­ ¾ß»ýÀüÅõÀÎÁö ±¸ºÐÇÏ´Â°÷
	{
...
void BattleUnitRenderer::Catch()
{
	...
	if (MyCatchEnd == false)
	{
		...
		{	
			...
			//»ïÇ×¿¬»êÀÚ ¾ËÆÄ°ªÀÌ 0º¸´Ù Å©¸é ÇØ´ç°ª, ¾Æ´Ï¶ó¸é 0°íÁ¤
...
void BattleUnitRenderer::TrainerOpening1()
{
	...
	{
		PlayerRenderer_->SetPivot({ -200.0f, 31.0f });//ÇÃ·¹ÀÌ¾î °ø½ÄÀ§Ä¡ °íÁ¤
		OpponentRenderer_->SetPivot({ 230.0f,-155.0f });//Àû Æ®·¹ÀÌ³Ê °ø½ÄÀ§Ä¡ °íÁ¤
		MoveSpeed = 0.0f;
		PlayerStop = true;
		FirstMove = false;
	}
...
{
	//±è¿¹³ª : Æ®·¹ÀÌ³Ê°¡ ¸ÕÀú Æ÷ÄÏ¸óÀ» ²¨³»°í ±× ´ÙÀ½¿¡"Go"¾Ö´Ï¸ÞÀÌ¼ÇÀÌ ½ÃÀÛµÊ
	{
		float Move = PlayerRenderer_->GetPivot().x;
		float BallMoveY = MonsterBallOP->GetPivot().y;
		float BallMoveX = MonsterBallOP->GetPivot().x;
		...
		if (BattleInter->GetPlayerEnd() == true)
		{
			//±è¿¹³ª : Æ®·¹ÀÌ³Ê°¡ ³ª°¡¸é¼­ ÇªÅ°¸ÕÀ» ²¨³¿
			//»ó´ë Æ®·¹ÀÌ³Ê´Â Æ÷ÄÏ¸óÀ» ²¨³Â´Ù! ÅØ½ºÆ®ÈÄ
			...
			//¿©±â±îÁö°¡ »ó´ë°¡ Æ÷ÄÏ¸óÀ» ²¨³»´Â ¾Ö´Ï¸ÞÀÌ¼Ç///////////////////////


			//¿©±âºÎÅÍ ³»°¡ Æ÷ÄÏ¸óÀ» ²¨³»´Â ¾Ö´Ï¸ÞÀÌ¼Ç//////////////////////////
			//¿©±â¼­ °¡¶ù! ²¿ºÎ±â! ½ÃÀÛ
			...
			{
				//Æ®·¹ÀÌ³Ê°¡ È­¸é¹Û¿¡ ³ª°¡¸é ²ô°í Go¾Ö´Ï¸ÞÀÌ¼Ç ½ÇÇà
				OpponentRenderer_->Off();
...
{
	//Áö±Ý Æ÷ÄÏ¸óÀÌ´ø ´ÙÀ½ Æ÷ÄÏ¸óÀÌ´ø ·£´õ·¯ º¯¼ö´Â PoeCurrentPokemon_·Î ÀçÅÁÇÑ´Ù
	PoeCurrentPokemon_->SetAlpha(0);

	//"NPC°¡ ´ÙÀ½ Æ÷ÄÏ¸óÀ» ²¨³Â´Ù!"ÅØ½ºÆ® Ãâ·Â
	//BattleInter->NextNPCTalk();//¹®Á¦ÀÇ ÇÔ¼ö//////////////////////////////////

	{
		if (/*¹«¾ð°¡ Á¶°ÇÀÌ °É·Á¾ßÇÔ NextBattleCheck°°ÀºboolÇÏ³ª ÀÖ¾î¾ßÇÒµí*/true)
		{
			//´ÙÀ½ ÇªÅ°¸Õ µîÀå(¸ó½ºÅÍº¼ ¿ÀÇÂ)

			CatchBallOpen->SetPivot({ 210.0f,-70.0f });
			CatchBallOpen->On();
			CatchBallOpen->ChangeAnimation("Open");

			MoveSpeed = 900.0f;//ÇªÅ°¸Õ ¾ËÆÄ°ª µµ´Â ¼Óµµ
			PlayerTime_2 += GameEngineTime::GetDeltaTime() * MoveSpeed;
			...
			{	//¾ËÆÄ°ª 255ÀÌ»óÀÌ¸é 255·Î °íÁ¤
				PoeCurrentPokemon_->SetAlpha(255);
				CatchBallOpen->Off();//¸ó½ºÅÍº¼ ²¨Áü
			}
			PoeCurrentPokemon_->SetPivot({ 230.0f,-105.0f });//ÇªÅ°¸Õ À§Ä¡ °íÁ¤
			//¿©±â±îÁö°¡ »ó´ë°¡ Æ÷ÄÏ¸óÀ» ²¨³»´Â ¾Ö´Ï¸ÞÀÌ¼Ç///////////////////////
			...
			if (BallLerp > 3.0f && Fighting == false)
			{	//¸í·ÉÃ¢ ON + µÒÄ©È¿°ú ½ÃÀÛ
				//ÀÌ¶§µµ ÅØ½ºÆ®Ãâ·ÂµÇ¸é¼­ ²¨Á®ÀÖÀ¸´Ï ÄÑÁö´Â°Ô ¸Â³ª?
				BattleInter->GetInterfaceImage()->On();
				BattleInter->GetSelect()->On();
				DoomChit();
				BattleInter->DoomChit();
